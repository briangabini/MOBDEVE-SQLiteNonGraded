...
class AddContactActivity : AppCompatActivity() {
    ...
    * */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val i = intent
        this.contactId = i.getLongExtra(IntentKeys.CONTACT_ID_KEY.name, -1)         // If there's no intent, the default value is -1
        ...
        this.viewBinding.selectBtn.setOnClickListener(View.OnClickListener {
            ...
            selectPictureIntent.action = Intent.ACTION_OPEN_DOCUMENT
            myActivityResultLauncher.launch(Intent.createChooser(selectPictureIntent, "Select Picture"))
        })
        ...
        this.viewBinding.addBtn.setOnClickListener(View.OnClickListener { view ->
            if (areFieldsComplete()) {
                ...
                executorService.execute {
                    ...
                    /*
                     * DONE: Handle the logic for knowing whether its an add or edit operation.
                    ...
                    if (contactId == -1L) {
                        ...
                        addIntent.putExtra(IntentKeys.FIRST_NAME_KEY.name, this.viewBinding.firstNameEtv.text.toString())
                        addIntent.putExtra(IntentKeys.LAST_NAME_KEY.name, this.viewBinding.lastNameEtv.text.toString())
                        addIntent.putExtra(IntentKeys.NUMBER_KEY.name, this.viewBinding.numberEtv.text.toString())
                        addIntent.putExtra(IntentKeys.IMAGE_URI_KEY.name, imageUri.toString())
                        ...
                        // Log intent values
                        Log.d("AddContactActivity", "Added Contact: ${this.viewBinding.firstNameEtv.text.toString()} ${this.viewBinding.lastNameEtv.text.toString()}")
                        // Log result code
                        Log.d("AddContactActivity", "Result Code: ${ResultCodes.ADD_RESULT.ordinal}")
                    }
                    /*
                     * DONE: Make sure to modify the setting of the result to an ADD_RESULT or
                     ...
                     * */
                        ...
                        editIntent.putExtra(IntentKeys.FIRST_NAME_KEY.name, this.viewBinding.firstNameEtv.text.toString())
                        editIntent.putExtra(IntentKeys.LAST_NAME_KEY.name, this.viewBinding.lastNameEtv.text.toString())
                        editIntent.putExtra(IntentKeys.NUMBER_KEY.name, this.viewBinding.numberEtv.text.toString())
                        editIntent.putExtra(IntentKeys.IMAGE_URI_KEY.name, imageUri.toString())
                        editIntent.putExtra(IntentKeys.CONTACT_ID_KEY.name, contactId)
