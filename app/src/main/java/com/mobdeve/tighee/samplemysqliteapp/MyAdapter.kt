...
class MyAdapter(private val contacts: ArrayList<Contact>, private val myActivityResultLauncher: ActivityResultLauncher<Intent>) : RecyclerView.Adapter<MyViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        ...
        /* Delete Logic
         * DONE: Supply the deletion logic in the onClick method
        ...
        myViewHolder.setDeleteBtnOnClickListener(View.OnClickListener { view ->
            ...
            executorService.execute {
                val myDbHelper = MyDbHelper(view.context)
                myDbHelper.deleteContact(contactId.toInt())

                // Remove contact from the contacts arraylist
                // Need to use the context of Main Activity since only the original thread that created a view hierarchy can touch its views
